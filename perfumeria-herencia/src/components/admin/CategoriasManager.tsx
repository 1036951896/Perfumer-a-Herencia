...
function CategoriaModal({
  ...
  onClose: () => void;
  onSave: (data: { id?: string; nombre: string; descripcion: string }) => Promise<void>;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onSave({ id: categoria?.id, nombre, descripcion });
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej: Diseñador, Nicho, Árabes"
            />
          </div>
...
export function CategoriasManager({ initialCategorias }: { initialCategorias: Categoria[] }) {
  ...
  const [modal, setModal] = useState<{ open: boolean; categoria: Partial<Categoria> | null }>({
    open: false,
    categoria: null,
  });

  const openCrear = () => setModal({ open: true, categoria: null });
  const openEditar = (cat: Categoria) => setModal({ open: true, categoria: cat });
  const closeModal = () => setModal({ open: false, categoria: null });

  const handleSave = async (data: { id?: string; nombre: string; descripcion: string }) => {
    ...
    if (isEdit) {
      setCategorias(prev =>
        prev.map(c => c.id === data.id ? { ...c, ...json.datos } : c)
      );
    } else {
      setCategorias(prev => [...prev, { ...json.datos, _count: { productos: 0 } }]);
    }
  };

  const handleDelete = async (cat: Categoria) => {
    if (cat._count.productos > 0) {
      alert(`No se puede eliminar "${cat.nombre}": tiene ${cat._count.productos} producto(s) asignado(s).`);
      return;
    }
  ...
  return (
    <div className="max-w-6xl">
      ...
      <header className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-medium text-gray-900 mb-1">Categorías</h1>
          <p className="text-sm text-gray-500">{categorias.length} categorías en catálogo</p>
        </div>
        <button
          onClick={openCrear}
            ...
            <tbody className="divide-y divide-gray-200">
              {categorias.map((cat) => (
                <tr key={cat.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-3 px-4 text-gray-600 max-w-xs truncate">{cat.descripcion || '—'}</td>
                  <td className="py-3 px-4 text-right text-gray-900">{cat._count.productos}</td>
